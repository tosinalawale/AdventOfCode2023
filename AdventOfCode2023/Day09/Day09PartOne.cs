    ...
    public class Day09PartOne
    {
        public static int CalculateResult(string[] input)
        {
            ...
            foreach (string line in input)
            {
                ...
                while (currentValueAtEachLevel[0] != numberSequence[^1])
                {
                    var oldValueAtEachLevel = currentValueAtEachLevel.Select(x => x).ToList();

                    for (int i = firstValueAtEachLevel.Count - 2; i >= 0; i--)
                    {
                        currentValueAtEachLevel[i] += oldValueAtEachLevel[i + 1];
                    }
                }

                var prevValueAtEachLevel = currentValueAtEachLevel.Select(x => x).ToList();

                for (int i = firstValueAtEachLevel.Count - 2; i >= 0; i--)
                {
                    currentValueAtEachLevel[i] += prevValueAtEachLevel[i + 1];
                }
